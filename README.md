...
- **侧边栏模式**：鼠标一离开页面区域就自动伪装；回来自动恢复（可关掉，改成手动退出）；可选「切标签页 / 窗口时也伪装」
- **页面不透明度**：40–100% 滑杆，只调脚本自绘的左栏 + 主区；V2EX 自己的页面和伪装视图、设置面板、灯箱这类浮层永远 100%
...
- **正文图片** → 默认渲染成小缩略图（260×170 上限），鼠标停上去在旁边浮出大图预览，
  点击仍然开灯箱
- **楼中楼引用卡片** → V2EX 的「楼中楼」就是回复开头 `@某人`。脚本会把它解析成一张
  引用卡片，直接显示引的是哪一楼、被引内容，可折叠、可一键跳到源楼层
...
设置面板 → **侧边栏模式** → 打开「侧边栏模式」后，**鼠标一离开页面区域**
（移到浏览器地址栏、别的窗口、dock 在侧边的 DevTools 也算）就自动做和连按两下 `Esc`
一样的事；鼠标回到页面自动恢复（离开后约 160ms 才动手 —— 用来滤掉鼠标划过页面内
跨源 iframe 这种假离开）。旁边两个独立开关：「切换标签页 / 窗口时也伪装」再加一个
触发源，「鼠标回来自动恢复」关掉后走开照藏、但回来不会自己退出（得自己按 `Esc` `Esc`
或 `Ctrl+Shift+H`）。三个开关都是改完立刻生效、不用刷新。

两条容易踩的语义：**它只撤自己触发的那次伪装** —— 手动按 `Esc` `Esc` / `Ctrl+Shift+H`
进的伪装，鼠标碰一下页面不会被弹出来；**设置面板开着时它不动手**（否则鼠标一移出去，
页面被藏起来，人就被锁在门外了）。`stealth` 关掉时这个模式整体失效（不再自动伪装）。

> 实现、扩展方式和已知边界（DevTools 停靠、触摸设备等）见
> [设置与隐蔽性扩展指南](docs/settings-and-stealth-guide.md)。
...
| | `pageOpacity` | `100` | **页面不透明度**（`40`–`100`，步长 `5`）：只作用于**脚本自绘的界面**（左栏 + 主区，含代码面板/输入框）；V2EX 自己的页面和伪装视图、设置面板、灯箱、toast 这类浮层永远 100% |
...
| **侧边栏模式** | `sidebarMode` | **`false`** | 鼠标一离开页面区域就自动伪装（和 `Esc`² 同一个视图）；**只撤它自己触发的那次**，手动按出的伪装不受影响 |
| | `sidebarAutoRestore` | **`true`** | 鼠标回来 / 标签页切回来时是否自动退出伪装；关掉后走开照藏，但要自己按 `Esc`² 才退出 |
| | `sidebarHideOnBlur` | **`false`** | 再加一个触发源：窗口失焦 / 标签页被切走时也伪装 |
...
| **楼中楼** | `quoteCard` | `true` | 把「@某人」渲染成引用卡片 |
| | `quoteOpen` | `true` | 引用卡片默认展开 |
...
三种加法（纯 CSS 变量类 / 开关类 / 自定义预览）都有可照抄的片段，见
[设置与隐蔽性扩展指南](docs/settings-and-stealth-guide.md#怎么再加一个设置)
—— 加完记得同步 `tools/test-jsdom.js` 里那几条写死的控件数量断言。

## 楼中楼引用卡片

V2EX 没有结构化的引用节点，「楼中楼」就是回复正文以 `@某人` 开头：

```html
<div class="reply_content">@<a href="/member/layxy">layxy</a> #8 无论输入来自什么模型…</div>
```

脚本把它解析成一张卡片：

![楼中楼引用卡片](docs/screenshots/quote-card.png)

- 头部 `❝ 引用 8 楼 · layxy`，**直接告诉你回的是谁**
- **自动定位到「该用户在此楼之前最后一次发言」那一楼**（V2EX 的楼中楼基本都是回最近那条），
  不是简单地指向他的任意一楼
- 提到楼主本人、而楼主此前没发过回复时，卡片指向主题正文，标签显示「楼主」
- `↗` 跳到源楼层并闪一下；点头部折叠 / 展开
- 引用里的图片降级成 `[图片]` 占位（原楼层已经有了，不重复显示）
- 展开时长引用限高 260px，超了卡片内部滚动，不会占满整屏

> **解析不到就不动它。** 如果被 `@` 的人在这段回复范围内没发过言（比如在更早的分页里），
> 脚本**不会**显示空卡片，而是把 `@某人` 原样留在正文里 —— 宁可少个功能，不能丢信息。
> 关掉这个功能（设置面板 → 楼中楼）也是同样处理。

在测试夹具 `ref/topic.html` 里，19 楼有 9 条是楼中楼，`ref/bigtopic.html` 是 100 楼里 25 条。
...
| `Ctrl/⌘ + ,` | 打开 / 关闭设置面板 |
| `Ctrl/⌘ + K` | 搜索 |
| 鼠标离开页面区域 | 自动伪装；回到页面自动恢复（两者都需在设置里打开「侧边栏模式」；只撤它自己触发的那次） |
...
## 与原脚本（[linux.do](https://github.com/czm15053/linuxdo-idea-ui) 版）的差异
...
`npm test` 在 jsdom 里加载**真实 V2EX 页面标记**（`ref/*.html` 抓取快照）跑 **923 条断言**：
...
- **侧边栏模式**（43 条）：页面内部移动不触发、**离开后立刻回来不触发（160ms 迟滞）**、
  真正移出页面才伪装（伪装时 rail `visibility: hidden` 且建出伪装视图）、重复离开不出错、
  回到页面自动恢复、**手动 `Esc`² 打开的伪装不会被鼠标撤掉**、设置面板开着时不伪装、
  关掉开关后不再伪装、默认不监听 `blur`、打开后失焦即伪装并能用 `mouseenter` /
  `visibilitychange` 恢复、**运行中打开/关闭 blur 触发源立即生效**；
  `sidebarAutoRestore=false` 时：离开照样伪装、回来不自动退、手动 `Esc`² 仍能退出、
  后续 blur 不会把手动那次伪装认领成自动触发
- **页面不透明度**：默认变量 = `1` 且不加 `v2cx-page-dim`、滑杆区间 `40~100` / 步长 `5`、
  拖动时变量变 `0.7` 且**不写存储**、读数显示 `70%`、松手才落盘、`dim` 规则
  **只覆盖脚本自绘的 rail / main**（明确断言不含原生 `#Top/#Wrapper/#Bottom`）、
  **伪装视图与浮层（设置面板 / 灯箱 / toast）也都不在名单里**、调回 100% 后 class 与变量都复位、
  `pageOpacity=0` 这类坏数据被收到滑杆下界 40%；另有 5 条**计算值**断言
  （rail / 主区算出来的 `opacity` = `0.7`、设置面板与 `#Wrapper` 仍是 `1`、
  页面层子元素不会被重复相乘）
...
- 楼中楼引用卡片：9 条全部解析出卡片、标题格式、折叠交互、跳转 + 高亮、
  被引楼层定位正确（第 9 楼 `@layxy` → 第 8 楼）、引用里的图片降级为 `[图片]`、
  **解析不到目标时不显示空卡片且 `@` 原样保留**、关掉功能后 9 条 `@` 全都在
...
docs/screenshots/*.png      README 用的效果图（含楼中楼引用卡片、设置面板）
docs/settings-and-stealth-guide.md
                            设置与隐蔽性扩展指南（不透明度分层、侧边栏模式、
                            怎么再加一个设置 / 自动反应）

ref/README.md               测试夹具说明（来源、内容、怎么重新抓）
ref/*.html                  真实 V2EX 页面快照，仅供测试

tools/test-jsdom.js         npm test        —— 923 条断言
